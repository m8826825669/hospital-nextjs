...
export const hrService = {
  async listEmployees(params: HrListParams): Promise<ApiListResponse<Employee>> {
    const response = await apiClient.get<ApiListResponse<Employee>>(
      "/hr/employees",
      { params }
    );
  ...
  async updateEmployee(id: string, payload: EmployeeFormValues): Promise<Employee> {
    const response = await apiClient.patch<Employee>(`/hr/employees/${id}`, payload);
    return response.data;
  },
  ...
  async listAttendance(
    params: HrListParams
  ): Promise<ApiListResponse<AttendanceRecord>> {
    const response = await apiClient.get<ApiListResponse<AttendanceRecord>>(
      "/hr/attendance",
      { params }
    );
    return response.data;
  ...
  async createLeave(payload: LeaveFormValues): Promise<LeaveRequest> {
    const response = await apiClient.post<LeaveRequest>("/hr/leaves", payload);
    return response.data;
  },
